...
{
	var alert_value=confirm(message);
	if(alert_value==true)
	{return true;}
	else
	{return false;}
...
{
	my_window=window.open(baseUrl+"/index.php?Theme=Black&Base=Api&Script=file_upload&api=1","file", "toolbar=no,scrollbars=no,resizable=yes,top=500,left=500,width=400,height=400");
	set_interval();
	}
...
function data_refresh()
{
  ...
  xmlhttp.open("GET", baseUrl+"/index.php?Theme=Black&Base=Api&Script=get_file&api=1&file_start="+file_start+"&file_limit="+file_limit, true);
  xmlhttp.send();
	}
	...
	function cont_hide(id)
{
  ...
  xmlhttp.open("GET", baseUrl+"/index.php?Theme=Black&Base=Api&Script=del_file&api=1&id="+del_id, true);
  xmlhttp.send();
	}
